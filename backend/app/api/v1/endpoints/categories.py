...
from ....crud.category import list_categories, create_category, get_category_by_name, search_categories
...
async def get_categories(q: str = Query(default=None), db: AsyncSession = Depends(get_db)):
    if q:
        return await search_categories(db, q)
    return await list_categories(db)
